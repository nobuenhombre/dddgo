...
// Query is a marker type used to identify Query structures.
// Embed this as an anonymous field in your Query structs to enable validation.
type Query struct{}

const (
	DeclaredName = "Query"
	MarkerField  = "_"
	FullPackage  = "github.com/nobuenhombre/dddgo/pkg/layers/infrastructure/interface-adapters/application/objects/queries"
)

// IsQueryTypeDeclaration checks if a struct type contains the Query marker field named "_".
//
// Parameters:
...
//
// Returns:
//   - true if the struct contains the Query marker named "_", false otherwise
func IsQueryTypeDeclaration(file *ast.File, structType *ast.StructType) bool {
	return helpers.IsSomeObjectTypeDeclaration(file, structType, FullPackage, MarkerField, DeclaredName)
}

// ValidateQueriesReport contains the results of value object validation analysis.
//
...
type ValidateQueriesReport struct {
	Types        map[string]bool
	Constructors map[string]*helpers.ConstructorInfo
	Violations   map[string]bool
}

// ValidateQueries analyzes Go source code to validate value object patterns.
//
...
//
// Returns:
//   - *ValidateQueriesReport: A detailed report containing found types, constructors, and violations
...
func ValidateQueries(rootPath string) (*ValidateQueriesReport, error) {
	types, err := helpers.FindTypeDeclarations(rootPath, IsQueryTypeDeclaration)
	if err != nil {
